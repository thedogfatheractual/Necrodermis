...
print_credit() {
    ...
    echo "  ╚══════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    echo -e "${G}  Necrodermis is built upon the framework of JaKooLit's${NC}"
    echo -e "${G}  Hyprland-Dots — one of the finest Hyprland configurations${NC}"
    echo -e "${G}  in existence. The wallpaper system, startup scripts, keybind${NC}"
    echo -e "${G}  framework, window rules, and core DE infrastructure are${NC}"
    echo -e "${G}  their work. We do not claim otherwise.${NC}"
    echo ""
    echo -e "${B}  ── JaKooLit — Hyprland-Dots ──${NC}"
    echo -e "${G}  https://github.com/JaKooLit/Arch-Hyprland${NC}"
    echo ""
    echo -e "${DG}  Necrodermis contributes: visual theme, SDDM login experience,${NC}"
    echo -e "${DG}  GTK/Qt styling, btop theme, icon recolour, and lore layer.${NC}"
    echo ""
